...
In order to develop a good database, we had to get a good overview of the workload our application could realistically face and the performance standards it would have to meet. It was crucial that an analysis of our database's expected workload was done and from that extract mechanisms, like triggers user functions and transactions, that assure the reliability and security of our database, but also indexes that will improve the performance and better the user's experience.

## 6.1. Database Workload

| Relation | Relation name | Order of magnitude | Estimated growth |
| -------- | ------------- | ------------------ | ---------------- |
| R01      | administrator | 1 (units)          | 0 (units) / day  |
| R02      | user          | 100 k (hundreds of thousands) | 100      (hundreds) / day  |
| R03      | unblock_appeal | 100 (hundreds)          | 1 (units) / day  |
| R04      | event | 10 k (tens of thousands)          | 10 (tens) / day  |
| R05      | tag | 100 (hundreds)          | 1 (units) / week  |
| R06      | post | 100 k (hundreds of thousands) | 100 (hundreds) / day |
| R07 | poll | 10 k (tens of thousands) | 10 (tens) / day |
| R08 | option | 10 k (tens of thousands) | 10 (tens) / day |
| R09 | comment | 100 k (hundreds of thousands) | 100 (hundreds) / day |
| R10 | rating | 100 k (hundreds of thousands) | 100 (hundreds) / day |
| R11 | file | 10 k (tens of thousands) | 10 (tens) / day |
| R12 | inquiry | 1 k (thousands) | 1 (units) / day |
| R13 | request | 1 k (thousands) | 1 (units) / day |
| R14 | invite | 1 k (thousands) | 1 (units) / day |
| R15 | report | 1 k (thousands) | 1 (units) / day |
| R16 | user_report | 1 k (thousands) | 1 (units) / day |
| R17 | comment_report | 1 k (thousands) | 1 (units) / day |
| R18 | event_report | 1 k (thousands) | 1 (units) / day |
| R19 | transaction | 10 k (tens of thousands) | 10 (tens) / day |
| R20 | event_cancelled_notification | 100 (hundreds) | 1 (units) / day |
| R21 | attendee | 100 k (hundreds of thousands) | 100 (hundreds) / day |
| R22 | vote | 100 k (hundreds of thousands) | 100 (hundreds) / day |
| R23 | event_cancelled_notification_user | 1 k (thousands) | 10 (tens) / day |
| R24 | tag_event | 10 k (tens of thousands) | 10 (tens) / day |
...
| Index | IDX01 |
| --- | --- |
| **Index relation** | attendee |
| **Index attribute** | id_user |
| **Index type** | Hash |
| **Cardinality** | High |
| **Clustering** | No |
| **Justification** | Table ‘attendee’  is very frequently accessed to gather either which events a certain user is attending or check if they are attending a certain event. The act of searching the attendees of a certain event is less frequent than searching the events that a user is attending. Since filtering is done by exact match we use an hash type index. The table itself is large, and the expected update frequency is very high, so no clustering is proposed. |
**SQL Code**
```sql
CREATE INDEX user_event_attendee ON attendee USING hash (id_user);
```

| Index | IDX02 |
| --- | --- |
| **Index relation** | comment |
| **Index attribute** | id_event |
| **Index type** | Hash |
| **Cardinality** | Medium |
| **Clustering** | No |
| **Justification** | Table ‘comment’ is frequently accessed to filter the comments of a given event and present them. Filtering is done by exact match, thus we use an hash type index. The expected update frequency is high so no clustering is proposed. |
**SQL Code**
```sql
...
| Index | IDX03 |
| --- | --- |
| **Index relation** | rating |
| **Index attribute** | id_comment |
| **Index type** | Hash |
| **Cardinality** | Medium |
| **Clustering** | No |
| **Justification** | Table ‘rating’ is frequently accessed to present the upvotes and downvotes in a given comment. The expected update frequency is high so no clustering is proposed. |
**SQL Code**
```sql
...
| **Index type** | GIN |
| **Clustering** | No |
| **Justification** | To provide full-text search features to look for events based on matching titles or description. The index type is GIN because the indexed fields are not expected to change often. |
**SQL Code**
```sql
...
| Trigger | TRIGGER01 |
| --- | --- |
| **Description** | Host cannot be an attendee. |
**SQL Code**
```sql
...
| Trigger | TRIGGER02 |
| --- | --- |
| **Description** | We can only have requests to private access events. |
**SQL Code**
```sql
...
| Trigger | TRIGGER03 |
| --- | --- |
| **Description** | Users can’t report themselves. |
**SQL Code**
```sql
...
| Trigger | TRIGGER04 |
| --- | --- |
| **Description** | The comment author can’t report their own comment. |
**SQL Code**
```sql
...
| Trigger | TRIGGER05 |
| --- | --- |
| **Description** | The event host can’t report their own event. |
**SQL Code**
```sql
...
| Trigger | TRIGGER06 |
| --- | --- |
| **Description** | A host cannot be invited to their own event. |
**SQL Code**
```sql
...
| Trigger | TRIGGER07 |
| --- | --- |
| **Description** | Whenever a user votes on an option in a poll the vote attribute is increased by one. |
**SQL Code**
```sql
...
| Trigger | TRIGGER08 |
| --- | --- |
| **Description** | User report cannot be a comment or event report. |
**SQL Code**
```sql
...
| Trigger | TRIGGER09 |
| --- | --- |
| **Description** | Comment report cannot be a user or event report. |
**SQL Code**
```sql
...
| Trigger | TRIGGER10 |
| --- | --- |
| **Description** | Event report cannot be a comment or user report. |
**SQL Code**
```sql
...
| Trigger | TRIGGER11 |
| --- | --- |
| **Description** | Comment author must be an attendee or host of the event the comment belongs to. |
**SQL Code**
```sql
...
| Trigger | TRIGGER12 |
| --- | --- |
| **Description** | Comment reader must be an attendee or host of the event the comment belongs to. |
**SQL Code**
```sql
...
| Trigger | TRIGGER13 |
| --- | --- |
| **Description** | Attendees cannot send a request to the event they’re participating in. |
**SQL Code**
```sql
...
| Trigger | TRIGGER14 |
| --- | --- |
| **Description** | Only an attendee of a certain event can vote on its polls. |
**SQL Code**
```sql
...
| Trigger | TRIGGER15 |
| --- | --- |
| **Description** | Attendees can vote at most in one option. |
**SQL Code**
```sql
...
| Trigger | TRIGGER16 |
| --- | --- |
| **Description** | Users cannot be invited to events they’re already part of. |
**SQL Code**
```sql
...
| Trigger | TRIGGER17 |
| --- | --- |
| **Description** | Increment number of attendees attribute in events. |
**SQL Code**
```sql
...
| Trigger | TRIGGER18 |
| --- | --- |
| **Description** | Decrement number of attendees attribute in events. |
**SQL Code**
```sql
...
| Trigger | TRIGGER19 |
| --- | --- |
| **Description** | Increment number of upvotes attribute in 
comments.
 |
**SQL Code**
```sql
...
| Trigger | TRIGGER20 |
| --- | --- |
| **Description** | Increment number of downvotes attribute in comments. |
**SQL Code**
```sql
...
| Trigger | TRIGGER21 |
| --- | --- |
| **Description** | Decrement number of upvotes attribute in comments. |
**SQL Code**
```sql
...
| Trigger | TRIGGER22 |
| --- | --- |
| **Description** | Decrement number of downvotes attribute in comments. |
**SQL Code**
```sql
...
| **Justification** | When a user wants to delete their account, it’s necessary to use a transaction to make sure all the steps are executed properly. The isolation level is Repeatable Read because we need to make sure that the  |
| **Isolation level** |  |
**SQL Code**
```sql
BEGIN TRANSACTION;
...
| Transaction | TRAN02 |
| --- | --- |
| **Description** | Add a user report. |
| **Justification** | When we add a user report to the database, we want to make sure that the report gets added to both the report table and the user_report table. The isolation level is … because ... |
| **Isolation level** |  |
**SQL Code**
```sql
BEGIN TRANSACTION;
...
| Transaction | TRAN03 |
| --- | --- |
| **Description** | Add an event report. |
| **Justification** | When we add an event report to the database, we want to make sure that the report gets added to both the report table and the event_report table. The isolation level is … because ... |
| **Isolation level** |  |
**SQL Code**
```sql
BEGIN TRANSACTION;
...
| Transaction | TRAN04 |
| --- | --- |
| **Description** | Add a comment report. |
| **Justification** | When we add a comment report to the database, we want to make sure that the report gets added to both the report table and the comment_report table. The isolation level is … because ... |
| **Isolation level** |  |
**SQL Code**
```sql
BEGIN TRANSACTION;
...
| Transaction | TRAN05 |
| --- | --- |
| **Description** | Join paid event. |
| **Justification** | When a user joins a paid event, we want to ensure that a payment is also added to the transaction table. The isolation level is … because ... |
| **Isolation level** |  |
**SQL Code**
```sql
BEGIN TRANSACTION;
...
| --- | --- |
| **Description** |  |
| **Justification** | Annunciate the cancelation of an event by sending event_canceled_notifications to every attendee of the event. -- TO BE ALTERED |
| **Isolation level** |  |
**SQL Code**
```sql
BEGIN TRANSACTION;
